...
def branch(gen, pos, special_chars, previous):    
    ...
    for m in moves.values():
        ...
        done = branch(gen, new_pos, special_chars, previous)
...
if __name__ == "__main__":
    ...
    while again:
        # * If you wanted smaller generations you could either remove enemy or key
        # * Or check if the product of the two are >4 rather than check both are >1
        while not (columns := input("Enter number of columns (must be >1): ")) or not columns.isdigit() or not ((columns := int(columns)) > 1): pass
        while not (rows := input("Enter number of rows (must be >1): ")) or not rows.isdigit() or not ((rows := int(rows)) > 1): pass
        ...
        while not gen_verified:
            ...
            for key in special_chars:
                while (pos := [randint(0,columns-1), randint(0,rows-1)]) == [0,0] or pos in special_chars.values() or sum(pos) < 0.25 * (columns+rows): pass
            ...
            gen_verified = branch(copy_gen, [0, 0], {"$":0, "^":0, "!": 0}, [[0,0]])
